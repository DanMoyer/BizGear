...
namespace Diagnostics.Tracing
{
    ...
    public unsafe sealed class ETWTraceEventSource : TraceEventDispatcher, IDisposable
    {
        ...
        {
            var now = DateTime.Now.ToFileTime() - 100000;     // used as the start time for real time sessions (sub 10msec to avoid negative times)
            ...
            {
                var fileBaseName = Path.GetFileNameWithoutExtension(fileOrSessionName);
                var dir = Path.GetDirectoryName(fileOrSessionName);
                ...
                for (var index = 0; index < allLogFiles.Count; index++)
                    logFiles[index].LogFileName = allLogFiles[index];
            }
            else
            {
            ...
            {
                handles[idx] = TraceEventNativeMethods.OpenTrace(ref logFiles[idx]);
                if (handles[idx] == TraceEventNativeMethods.INVALID_HANDLE_VALUE)
                    Marshal.ThrowExceptionForHR(TraceEventNativeMethods.GetHRForLastWin32Error());

                // Start time is minimum of all start times 
                if (logFiles[idx].LogfileHeader.StartTime < sessionStartTime100ns)
                    sessionStartTime100ns = logFiles[idx].LogfileHeader.StartTime;
                // End time is maximum of all start times
                if (logFiles[idx].LogfileHeader.EndTime > sessionEndTime100ns)
                    sessionEndTime100ns = logFiles[idx].LogfileHeader.EndTime;

                // TODO do we even need log pointer size anymore?   
                // We take the max pointer size.  
                if ((int)logFiles[idx].LogfileHeader.PointerSize > pointerSize)
                    pointerSize = (int)logFiles[idx].LogfileHeader.PointerSize;

                eventsLost += (int)logFiles[idx].LogfileHeader.EventsLost;
            }
            ...
            var ver = (int) logFiles[0].LogfileHeader.Version;
            ...
            Kernel.ProcessStartGroup += delegate(ProcessTraceData data)
            {
                ...
                var path = data.KernelImageFileName;
                var startIdx = path.LastIndexOf('\\');
